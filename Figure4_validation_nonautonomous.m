...
% for a non-autonomous model corresponding to Figure 4 in Z. Ahsan, H.
...
if exist('sys_forced', 'file')~=2
  syms x1 x2 x3 Omega omega
  f = [-Omega*x2+x1*(1+sqrt(x1.^2+x2.^2)*(cos(x3)-1)); ...
    Omega*x1+x2*(1+sqrt(x1.^2+x2.^2)*(cos(x3)-1)); ...
    omega];
  forced = sco_sym2funcs(f, {[x1; x2; x3], [Omega omega]}, {'x', 'p'}, ...
    'filename', 'sys_forced');
else
...
N = 10; nsegs = 2*N+1; xdim = 3; pdim = 2;
...
figure
coco_plot_sol('single_torus','torus1','t','x','x')
grid on
box on
...
prob = coco_prob;
prob = coco_set(prob, 'coll', 'MXCL', false, 'NTST', 30);
